...
class GraphPipeline:
    ...
    def run(self, file_path: str | None = None, target: str | int | None = None, **context: Any) -> Any:
        ...
        Returns:
            The serialized string representation of the trimmed graph, or the
            last component output if a custom terminal component is used.
        """
    ...
    def dry_run(self, file_path: str | None = None, target: str | int | None = None) -> list[dict[str, Any]]:
    ...
    def _resolve_target_node_id(self, graph: CodeGraph, target: str | int) -> str:
        ...
        if not target_nodes:
            raise ValueError(
                f"No nodes found matching target {target!r} under granularity {granularity.granularity_name!r}"
            )
    ...
    def _sync_legacy_attributes(self) -> None:
        ...
        self.parser = next((c for c in self._components if isinstance(c, BaseParser)), None)
        self.trimmer = next((c for c in self._components if isinstance(c, BaseTrimmer)), None)
        self.serializer = next((c for c in reversed(self._components) if isinstance(c, BaseSerializer)), None)